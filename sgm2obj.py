        ...
        for _ in range(num_materials):
            ...
            for _ in range(uv_count):
                ...
                for _ in range(image_count):
                    ...
                    texname = struct.unpack(f"<{texname_len}s", file.read(texname_len))[
                        0
                    ].decode("utf_8")
            ...
            materials.append(
                {"material_id": material_id, "uv_data": uv_data, "colors": colors}
            )
    ...
    with open(filename, "w") as f:
        ...
        for i, m in enumerate(meshes):
            ...
            if m["material_id"] is not None and m["material_id"] < len(materials):
                f.write(
                    f'usemtl {materials[m["material_id"]].get("material_id", "default")}\n'
                )
            else:
            ...
            for i in range(0, len(indices), 3):
                if len(m["vertices"][0][2]) > 0:
                    ...
                    f.write(
                        f"f {indices[i] + 1}//{indices[i] + 1} {indices[i + 1] + 1}//{indices[i + 1] + 1} {indices[i + 2] + 1}//{indices[i + 2] + 1}\n"
                    )
...
def main():
    ...
    parser.add_argument(
        "--texture", type=str, help="name of texture file if there is only one texture"
    )
